...
def busquedaRuta():
    ...
    if len(result) > 0:
        ...
        if len(intermedio) > 0:
            ...
            tiempoDos = dt.timedelta(hours=float(result3[0][3][0:2]), minutes=float(
                result3[0][3][3:5]), seconds=float(result3[0][3][6:8]))
        ...
        print(
            f"NO SE ENCONTRARON RUTAS PARA VIAJAR DE {origen} - {destino} EN {fmt_tiempo_salida}")
    ...
    if len(result) != 0:
        if len(intermedio) != 0:
            ...
            if choice == 1:
                ...
                lugar_salida = result[choiceDirecta-1][1]
                lugar_llegada = result[choiceDirecta-1][2]
                duracion = result[choiceDirecta-1][5]
                precio = result[choiceDirecta-1][6]
                print(f"SU RUTA ELEGIDA ES {choiceDirecta}")
                frut.buyTickets(idDirecta, lugar_salida, lugar_llegada,
                                intermedioNull, duracion, precio)
                # return idDirecta
            if choice == 2:
                print("OPCION DE RUTA CON ESCALAS")
                print(
                    f"SU RUTA ELEGIDA ES {result2[0][0]}-{result3[0][0]}-{result3[0][1]}")
                idEscalas = [intermedio1, intermedio2]
                lugar_escala = result3[0][0]
                lugar_salida = result2[0][0]
                lugar_llegada = result3[0][1]
                duracion = duracionViaje
                precio = precioEscala
                frut.buyTickets(idEscalas[0], lugar_salida,
                                lugar_llegada, idEscalas[1], duracion, precio)
                # return idEscalas
        else:
            print("OPCION DE RUTA DIRECTA")
            while True:
